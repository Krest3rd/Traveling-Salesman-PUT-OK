...
def evaluate_params(distance_mat, alpha, beta, rho, q, runs=5):
...
def search_1d(distance_mat, param_name, current_params, search_range, steps=5):
    ...
    for v in values:
        ...
        avg, best = evaluate_params(distance_mat, **test_params)

        print(f"  Testing {param_name}={v:.4f} → avg={avg:.4f}")
        ...
        if best < best_len:
            ...
            with open("aco_log_coordinate_descent.txt", "+a") as f:
                f.write(f"New best length: {best_len:.4f} with params: {test_params} and avg={avg:.4f}\n")

    print(f"  → Best {param_name} = {best_params[param_name]:.4f} (avg={best_avg:.4f})")
...
def coordinate_descent(distance_mat, max_cycles=10):
    ...
    for cycle in range(max_cycles):
        ...
        for param in ["alpha", "beta", "rho", "q"]:
            print(f"\nOptimizing {param}...")
            params, best_avg = search_1d(distance_mat, param, params, ranges[param])

        print(f"\nCycle {cycle+1} best avg = {best_avg:.4f}")

        # Stopping condition: no improvement
        if best_avg >= prev_best - 1e-6:
            print("\nNo improvement detected. Stopping coordinate descent.")
            break
        
        with open("aco_log_coordinate_descent.txt", "+a") as f:
            f.write(f"Cycle {cycle+1}: params={params}, best_avg={best_avg:.4f}\n")
        prev_best = best_avg
